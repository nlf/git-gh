...
char* bold(char* str) {
    ...
    char* boldend = "\x1b[0m";
    int str_len = strlen(boldstart) + strlen(boldend) + strlen(str) + 1;
    char* newstr = (char*)calloc(sizeof(char), str_len);;
...
void print_issue(struct json_object* issue, char* token, char* repo) {
    ...
    if (comments > 0) {
        ...
        int len = json_object_array_length(comments);
        int i;
        ...
        for (i = 0; i < len; i++) {
            ...
            user = jsonh_get_string(comment, "user.login");
            body = jsonh_get_string(comment, "body");
            printf("%s:\t%s\n\n", bold(user), body);
        }
    }
}
